              ...
              <template>
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle
                    v-html="`سعر الوحدة: ${data.item.rate}`"
                  ></v-list-item-subtitle>
                </v-list-item-content>
              </template>
            </template>
                ...
                <v-row class="ma-0 pa-0">
                  ...
                  >
                    <v-text-field
                      ...
                      outlined
                      color="primary"
                      :label="frappe._('رقم الباتس. الكمية المتاحة')"
                      background-color="white"
                      hide-details
                  ...
                  >
                    <v-text-field
                      ...
                      outlined
                      color="primary"
                      :label="frappe._('تاريخ صلاحية رقم الباتش')"
                      background-color="white"
                      hide-details
                      ...
                      <template v-slot:item="data">
                        <template>
                          <v-list-item-content>
                            ...
                            <v-list-item-subtitle
                              v-html="
                                `الكمية المتاحة  '${data.item.batch_qty}' - تاريخ انتهاء الصلاحية ${data.item.expiry_date}`
                              "
                            ></v-list-item-subtitle>
                          </v-list-item-content>
                        </template>
          ...
          <v-row no-gutters class="pa-1 pt-9 pr-1">
            <v-col cols="6" class="pa-1">
              <v-text-field
                ...
                :label="frappe._('الكمية الإجمالية')"
                outlined
                dense
                readonly
                hide-details
  ...
  data() {
    return {
      ...
      items_headers: [
        ...
        { text: __('الكمية'), value: 'qty', align: 'center' },
        { text: __('وحدة القياس'), value: 'uom', align: 'center' },
        { text: __('سعر الوحدة'), value: 'rate', align: 'center' },
        { text: __('القيمة'), value: 'amount', align: 'center' },
        { text: __('هل هذا عرض؟'), value: 'posa_is_offer', align: 'center' },
      ],
    };
  },
      ...
      this.items.forEach((item) => {
        if (this.stock_settings.allow_negative_stock != 1) {
          if (
            ...
            evntBus.$emit('show_mesage', {
              text: __(
                `الكمية الموجودة '{0}' للصنف '{1}' غير كافية`,
        ...
        if (item.qty == 0) {
          evntBus.$emit('show_mesage', {
            text: __(`الكمية الصنف '{0}' غير مسموج ان تكون (0)`, [
              item.item_name,
            ]),
            color: 'error',
          });
        ...
        if (
          ...
          evntBus.$emit('show_mesage', {
            text: __(`الحد الأقصى للخصم للصنف '{0}' هي {1}%`, [
              item.item_name,
              item.max_discount,
            ]),
            color: 'error',
        ...
        if (item.has_serial_no) {
          if (
            ...
            evntBus.$emit('show_mesage', {
              text: __(`الرقم التسلسلي المختار للصنف {0} غير صحيح`, [
                item.item_name,
              ]),
              color: 'error',
            });
